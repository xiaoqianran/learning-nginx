...
export const Route = createRootRoute({
  head: () => ({
    meta: [
      ...
      {
        title: "Nginx 实战学习 v1 · 配置路径",
      },
      {
        name: "description",
        content:
          "Nginx 中文交互教程：清晰学习路径、文档地图、讲解+配置+Demo、Catppuccin 主题与配置工坊。",
      },
    ],
    links: [
...
const CTP_BOOT =
  "(function(){try{var f=localStorage.getItem('nginx-learn-ctp-flavor');var a=localStorage.getItem('nginx-learn-ctp-accent');var okF=['mocha','macchiato','frappe','latte'];var okA=['green','mauve','blue','lavender','sapphire','teal','peach','pink'];if(okF.indexOf(f)<0)f='mocha';if(okA.indexOf(a)<0)a='teal';document.documentElement.setAttribute('data-ctp-flavor',f);document.documentElement.setAttribute('data-ctp-accent',a);}catch(e){document.documentElement.setAttribute('data-ctp-flavor','mocha');document.documentElement.setAttribute('data-ctp-accent','teal');}})();";
