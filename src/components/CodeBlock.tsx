...
function colorLine(line: string) {
  ...
  const parts = line.split(
    /(\b(?:server|location|upstream|http|events|map|limit_req_zone|limit_conn_zone|if|return|rewrite|include|worker_processes|listen|server_name|root|alias|proxy_pass|try_files|ssl_certificate|ssl_certificate_key|add_header|gzip|expires)\b|'[^']*'|"[^"]*"|\{|\})/g,
  );
  return parts.map((p, i) => {
    if (!p) return null;
    if (
      /^(server|location|upstream|http|events|map|limit_req_zone|limit_conn_zone|if|return|rewrite|include|worker_processes|listen|server_name|root|alias|proxy_pass|try_files|ssl_certificate|ssl_certificate_key|add_header|gzip|expires)$/.test(
        p,
      )
    ) {
      return (
